...
class SimpleEnv:
    ...
    def render(self, teleop=False):
        ...
        if teleop:
            # ====== 六维力显示（核心新增部分） ======
            ft = self.get_ee_ft()   # [fx,fy,fz,tx,ty,tz]
            labels = ["Fx", "Fy", "Fz", "Tx", "Ty", "Tz"]

            for i in range(6):
                self.env.viewer.add_overlay(
                    text1 = labels[i],
                    text2 = f"{ft[i]:.2f}",
                    loc   = 'bottom left'
                )
    ...
    def get_ee_ft(self):
        '''
        get the end effector force/torque readings
        returns:
            ft: np.array, force/torque readings of the end effector
            [fx,fy,fz,tx,ty,tz]
        '''
        f = self.env.get_sensor_value("ft_force")    # (3,)
        t = self.env.get_sensor_value("ft_torque")   # (3,)
